...
cursors and their options live in `TriasDev.Tabular.Csv`, `TriasDev.Tabular.Xlsx` and `TriasDev.Tabular.Ods`. The folders
...
- **Abstractions** — `ITabularCursor` is the only format-aware seam; everything above is written
  ...
  or ods when the zip's first entry is the OpenDocument `mimetype`), never from its extension.
